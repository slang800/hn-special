...
if (ignore.indexOf(location.pathname) === -1) {
  ...
  if (container) {
    ...
    _.$('span.comment').forEach(function (elem) {
      ...
      if (typeof img !== 'undefined' && img.tagName.toLowerCase() === 'img') {
...
function stripAttributes (data) {
  ...
  set.forEach(function (elem) {
    ...
    for (var i = 0; i < attrs.length; i++) {
      ...
      if (['colspan', 'class', 'id', 'type', 'name', 'value', 'title'].indexOf(attr) !== -1) continue
      names.push(attr)
    }
...
HNSpecial.subscribe('new links', function (data) {
